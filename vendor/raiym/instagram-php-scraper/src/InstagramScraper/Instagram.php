...
class Instagram
{
    ...
    public function getMedias($username, $count = 20, $maxId = '')
    {
        ...
        $medias = $this->getMediasByUserId($account->getId(), $count, $maxId);
        return compact('account', 'medias');
    }
     ...
     * @throws InstagramNotFoundException
     */
    public function getPaginateMedias($username, $maxId = '', $accountId = false)
    {
        $account = $accountId? $accountId: $this->getAccount($username);
        $hasNextPage = true;
        $medias = [];
        ...
        $variables = json_encode([
            'id' => $accountId? (string) $accountId: (string) $account->getId(),
