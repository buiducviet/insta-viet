...
var App = function() {
    ...
    var templateOptions = function() {
        ...
        $('a', colorList).click(function(e){
            ...
            if (cookies) {
                var set = Cookies.set('optionThemeColor', themeColor, {expires: 365});
            }
        });
...
$(document).ready(function(){
    ...
    Upload.prototype.doUpload = function () {
        ...
        if ($('.ajax-upload #name-file:checked').length) {
        ...
        $.ajax({
            ...
            success: function (responseJSON) {
                if (responseJSON.status) {
                    var img = '<div class="col-md-3 image-item"><img src="'+responseJSON.link+'" class="img-responsive">';
                    img += '<input type="text" class="form-control" value="'+responseJSON.link+'"></div>';
    ...
    $('.ajax-upload [name="file"]').on("change", function (e) {
        $.each($(this)[0].files, function(i, file){
            var uploads = [];
            uploads[i] = new Upload(file);
            uploads[i].doUpload();
        });
