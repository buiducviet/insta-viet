...
$(function(){
    ...
    $("form.ig-search").submit(function(event){
        event.preventDefault();
        var form = $(this);
        if (form.find('[name="q"]').val().length) {
            window.location = form.attr('action') + '/' + encodeURIComponent(form.find('[name="q"]').val()).replace(/%20/g, "+").replace(/%2F/g, "+")
        }
    });

    $('.site-nav li a').each(function(){
        var e = $(this);
        if (e.hasClass('home-url')) {
            if (window.location.href == e.attr('href')+'/') {
                e.parent('li').addClass('active');
            }
        }else if (window.location.href.indexOf(e.attr('href')) > -1) {
            e.parent('li').addClass('active');
        }
    });

    $('#captcha_refresh').on('click', function () {
        $('#captcha_refresh .fa').addClass('fa-spin');
        var config = $('#captcha_src').data('refresh-config');
        $.ajax({
            method: 'GET',
            url: '/get_captcha/' + config,
        }).done(function (response) {
            $('#captcha_src').prop('src', response);
            $('#captcha_refresh .fa').removeClass('fa-spin');
        });
    });
});
